...
| `research/20260919-round4-evidence-presentation/upstream_sources/` | Five unmodified IsaacLab Python sources (converter, observation producer, observation-name configuration, rigid-object data and frame-transformer data), their repository license and an author-prepared source manifest are bundled to substantiate RD4 field conventions; this is source verification, not a new replay | BSD-3-Clause, IsaacLab Project Developers; original headers and `LICENSE-IsaacLab.txt` retained. `SOURCE_MANIFEST.json` binds each exact source URL, revision, byte count, SHA-256 and relevant line ranges. |
