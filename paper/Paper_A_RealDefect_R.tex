...
\textbf{Defect resources and oracles.} Defects4J couples historical faulty/fixed program versions with exposing tests \cite{11}. Jahangirova et al. examine source correspondence and reproducibility in deep-learning fault benchmarks \cite{30}; Shiri Harzevili et al. benchmark library fuzzers against real TensorFlow and PyTorch faults \cite{31}. RealDefect-R exposes semantic references and replay scope rather than evaluating fuzzer effectiveness. It focuses on data whose local representation remains valid while a physical or temporal relation is wrong, and retains a case whose complete criterion fails after repair. The oracle problem explains the need for a justified reference against which an output is judged \cite{12}. Here that reference and the execution scope are published with each result.\par
...
\bibitem{13} RealDefect-R contributors. Sanitized replay artifact, version \texttt{public-v1.0.1-20260919}, 2026. [Online]. Available: \url{https://github.com/xch3177-publicrepo/RealDefect-R/releases/tag/public-v1.0.1-20260919}.
