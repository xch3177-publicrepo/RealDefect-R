...
Although public issue trackers document many such failures, a URL alone is insufficient for reuse. Reuse requires the source version, the identity of the inputs read, the historical calculation exercised, the reference information on which the verdict depends, the complete decision rule, and its execution scope. \mbox{RealDefect-R} supplies that chain for eight cases and preserves the measured outcomes, including one that fails. It makes three contributions.\par
...
\section{The \mbox{RealDefect-R} Resource}
...
At collection scale, the question is whether coherent global episode boundaries can coexist with per-record summaries that retain identifiers from elsewhere. Here, an \textbf{episode} is one recorded demonstration and a \textbf{frame} is one recorded time step. LeRobot v3.0 stores these steps in Parquet files and keeps identifiers, frame ranges and summary statistics in a separate per-episode table; the audit asks whether the two layers agree.\par

Cosmos3-DROID's data card states that it converts raw DROID---collected across 18 laboratories at 13 institutions \cite{22}---to LeRobot v3.0 and re-releases it under NVIDIA's name \cite{21}. Its counts therefore come from raw DROID and differ from the DROID paper's trajectory totals. Its \textbf{success} and \textbf{failure splits} partition episodes by recorded outcome, not by audit verdict. We audit every episode-metadata shard (file partition) in both splits at revision \texttt{dabaaffe428d\allowbreak{}67cf93fd355b\allowbreak{}82658934ee59\allowbreak{}bfec}: six episode shards and six auxiliary, or \textbf{companion}, files, verified against complete SHA-256 digests and processed by the original collection audit rerun unchanged. The population is 71,907 records representing 22,412,712 declared frames.\par
...
Taken together, the replays and collection audit identify the reference information needed to check meaning across data hand-offs. These observed dependencies motivate the following practices for publishers and consumers to carry that information with the data.\par
...
\bibitem{13} \mbox{RealDefect-R} contributors. Sanitized replay artifact, version \texttt{public-v1.1.4-pre-llm-20260919}, 2026. [Online]. Available: \url{https://github.com/xch3177-publicrepo/RealDefect-R/releases/tag/public-v1.1.4-pre-llm-20260919}.
