...
\begin{document}
\title{RealDefect-R: Replayable Evidence for Semantic Verification in Shared Robot-Data Pipelines}
\maketitle
\begin{abstract}
...
Three observations guide the evidence:\par

\begin{itemize}

\item \textbf{References:} conventions, calibration anchors, temporal correspondence, cross-layer identity and population statistics define the case rules (Sections III-IV).

\item \textbf{Portability:} the evaluated declared-schema suite separates 1/7 pairs; case-informed rules separate 7/7 in the same validation host (Table II).

\item \textbf{Re-release:} coherent global numbering coexists with laboratory-local episode summaries (Section V).
...
Table I summarizes each operation, execution scope, tested local property, reference requirement and outcome. The registry supplies source revisions, local checks and complete decision rules. Two distinctions matter when reading the counts.\par
...
Fig. 2 shows identity-summary and task-text disagreements separately for each split. The frame checks and controlled consumer replay below address distinct relations.\par
...
\bibitem{13} RealDefect-R contributors. Sanitized replay artifact, version \texttt{public-v1.0.2-20260919}, 2026. [Online]. Available: \url{https://github.com/xch3177-publicrepo/RealDefect-R/releases/tag/public-v1.0.2-20260919}.
